...
```
npx github:Booyaka101/pal-schema-collect collect --dir <your Mods/PalSchema/schemas/raw folder> --submit
```
...
Needs Node ≥ 18; `--submit` uses your GitHub login (`gh` CLI or `GH_TOKEN`). Without `--submit` it's a dry run, and `validate` never touches the network.
